...
vec4 effect(vec4 color, sampler2D texture, vec2 uv, vec2 pc) {
    ...
    vec4 c = vec4(0.0);
    c.r = Texel(texture, uv + vec2(x_offset, 0.0) + vec2(rgb_offset*0.003, 0.0)).r;
    c.g = Texel(texture, uv + vec2(x_offset, 0.0)).g;
    c.b = Texel(texture, uv + vec2(x_offset, 0.0) - vec2(rgb_offset*0.003, 0.0)).b;
    c.a = Texel(texture, uv).a;
    float scan = sin(uv.y * 720.0) * 0.5 + 0.5;
    scan = mix(1.0, scan, scanlines);
    return color * c * scan;
}
